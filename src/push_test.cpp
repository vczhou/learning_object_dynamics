...
bool push_and_store(ros::NodeHandle n, double velocity){
    ROS_INFO("About to push object and store data");
    ...
    ROS_INFO("Finished creating directories");
    pressEnter();

    // Store a point cloud before the action is performed
    ROS_INFO("About to store original point cloud");
    storePointCloud();
    pressEnter();

    // Start recording data
    ROS_INFO("Start recording data");
    startSensoryDataCollection();

    // Push forward
    ROS_INFO("About to push");
    pushForward(velocity, 1.0, c_vel_pub_);

    // Stop recording data
    ROS_INFO("Stop recoring data");
    stopSensoryDataCollection();                

    // Store a point cloud after the action is performed
    ROS_INFO("Store ending point cloud");
    storePointCloud();

    ROS_INFO("Press enter we've finished this trial :D");
    pressEnter();
}
...
int main(int argc, char **argv){
    ...
    double velocity = getNumInput("Entrer Velocity to push with: ");

    push_and_store(n, velocity);
